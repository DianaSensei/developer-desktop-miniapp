// Plugin dịch vụ (tier B) cho Container Manager. Port từ `container_tool.rs`
// của developer-desktop-utils (bản built-in cũ, "Tier A") — Tier A đã bị xoá
// hẳn khỏi repo đó (Phase 2 Bước 4/5 cắt hẳn sang sidecar này); repo này giờ
// là bản triển khai duy nhất còn tồn tại. Xem
// docs/decisions/architecture/platform-plugin-architecture.md (repo
// developer-desktop-utils) mục "Ba tier" và "Cách ly dữ liệu" cho bối cảnh
// đầy đủ, và CHANGELOG.md của repo đó cho ghi chú dành cho người dùng.
//
// Khác với bản Tier A cũ đúng những chỗ là hệ quả của việc
...
//     `plugin-data/container-manager/connections.json` mà Tier A cũ dùng.
//     Quyết định có chủ ý KHÔNG di trú dữ liệu cũ sang vị trí mới (xem "Cách
//     ly dữ liệu" trong ADR) — người dùng nâng cấp từ bản built-in cũ cần tự
//     nhập lại cấu hình kết nối (socket path) một lần sau khi cài plugin.
...
// (Tier A cũ, đã bị xoá khỏi developer-desktop-utils) từng dùng. Cách ly
// bằng thư mục, không phải bằng tên file — xem module doc comment ở trên và
// ADR mục "Cách ly dữ liệu".
