...
// `containerApi` is a thin wrapper over `sdk.service.call` to the sidecar
// (this repo's `sidecar/`, shipped as `devtool-svc-container`), which
// reads/writes a JSON file in its own service-data dir, so it
...
import { CONTAINER_MCP_TOOLS } from './mcpTools';
...
export function useMcpBridge(state: ContainerToolState, enabled = true): void {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await sdk.native.invoke('mcp_register_tools', { pluginId: sdk.id, tools: CONTAINER_MCP_TOOLS });
