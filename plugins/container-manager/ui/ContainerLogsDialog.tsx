...
/** See the call site: viewport-relative sizing has to be inline for a plugin,
 *  because the host's Tailwind build never sees this file. */
const LOGS_BOX_STYLE = { height: '68vh', maxHeight: 'calc(100vh - 13rem)' } as const;
          ...
          <>
            ...
            <div className="flex min-w-0 flex-col gap-2 overflow-hidden">
              ...
              {/* Tall by design, but never taller than the window minus the
                  ...
                  Inline style, not `h-[68vh] max-h-[calc(100vh-13rem)]`: a
                  plugin bundle ships no CSS of its own and reuses the host
                  app's compiled Tailwind sheet, which is generated by scanning
                  the HOST's `src/**` only. An arbitrary-value class that no
                  host file also uses therefore has no rule at all — these two
                  had none, so this box fell back to `height: auto`, the log
                  list grew with every line that streamed in, and the dialog
                  ran off the top and bottom of the screen with its own toolbar
                  out of reach. See LOGS_BOX_STYLE above and the "Styling" note
                  in the repo README. */}
              <div className="flex flex-col" style={LOGS_BOX_STYLE}>
                <LogsPanel
                  key={target.Id}
