...
export interface KafkaConsumerSession {
  /** Host-side stream handle (null until consumeStart resolves). */
  subscription: { stop(): Promise<void> } | null;
  brokerId: string;
  topic: string;
  from: ConsumeFrom;
...
export const kafkaConsumerStore = {
  async start(brokerId: string, topic: string, from: ConsumeFrom): Promise<void> {
    ...
    const session: KafkaConsumerSession = {
      subscription: null, brokerId, topic, from, messages: [], received: 0, paused: false, bufferedWhilePaused: 0, startedAt: Date.now(), starting: true,
    };
    sessions.set(k, session);
    emit();

    try {
      const subscription = await kafkaApi.consumeStart(
        { configId: brokerId, topic, from },
        (msg) => { if (sessions.has(k)) enqueue(k, msg); },
      );
      const s = sessions.get(k);
      if (!s) { subscription.stop().catch(() => {}); return; } // stopped mid-start
  ...
  async stop(brokerId: string, topic: string): Promise<void> {
    ...
    discardPending(k);
    emit();
    if (s.subscription) { try { await s.subscription.stop(); } catch { /* ignore */ } }
  },
